...
for (const marker of ['style.css?v=1.0.7', 'readability.css?v=1.0.7', 'visualizer.css?v=1.0.7', 'three.min.js?v=1.0.7', 'app.js?v=1.0.7']) {
  if (!html.includes(marker)) throw Error(`missing 1.0.7 cache key ${marker}`);
}

const app = execFileSync('unzip', ['-p', pkg, 'static/app.js'], { encoding: 'utf8' });
for (const marker of ['Math.min(50,queueSongs.length)', 'trackIndexes(queueSongs.length,center)', 'stateQueue(state)', 'playerId(card.song)', 'randomSlots', 'trackLayout', 'mobileScale', 'wheel', 'volume-slider', 'setVolume', 'keydown', 'setPlayMode', '--visualizer-rgb']) {
...
if (manifest.version !== '1.0.7') throw Error('invalid release version');
...
if (!main.includes('songloft-now-playing 1.0.7 initialized')) throw Error('stale main entry identity');

console.log('songloft-now-playing 1.0.7 package identity, description, cache keys, hashes, structure, and runtime markers are valid.');
