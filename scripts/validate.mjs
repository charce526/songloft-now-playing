...
for (const marker of ['style.css?v=1.0.18', 'readability.css?v=1.0.18', 'visualizer.css?v=1.0.18', 'three.min.js?v=1.0.18', 'app.js?v=1.0.18']) {
  if (!html.includes(marker)) throw Error(`missing 1.0.18 cache key ${marker}`);
}

const app = execFileSync('unzip', ['-p', pkg, 'static/app.js'], { encoding: 'utf8' });
for (const marker of ['randomCardLimit(){return 50;}', 'randomCardLimit()-1', 'randomSlotsBySong', 'reconcileQueue', 'nearestLoopFocus', 'randomStage', 'optimisticPlaying=nextPlaying', 'optimisticSong=id', 'flipTarget=nextPlaying?0:1', 'new THREE.Euler', 'c.scatterRot.clone()', 'function toggleCurrent()', 'function statePlaying', 'resetRandomPose', 'nextMode=', 'tapDistance', 'hitCard', 'cardBoundsHit', 'focus=targetFocus=selected', 'visible=isMobile?a<2.5:a<6', 'dragBase=isMobile?165:185', 'displayPlaying', 'trackIndexes(queueSongs.length,center)', 'stateQueue(state)', 'playerId(card.song)', 'randomSlots', 'trackLayout', 'mobileScale', 'candidates.sort', 'wheel', 'volume-slider', 'setVolume', 'keydown', 'setPlayMode', 'onStateChange', '--visualizer-rgb']) {
...
if (manifest.version !== '1.0.18') throw Error('invalid release version');
...
if (!main.includes('songloft-now-playing 1.0.18 initialized')) throw Error('stale main entry identity');

console.log('songloft-now-playing 1.0.18 package identity, description, cache keys, hashes, structure, and runtime markers are valid.');
